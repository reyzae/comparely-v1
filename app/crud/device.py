...
def get_devices(
    ...
    if search:
        ...
        search_filter = models.Phone.name.ilike(
            search_pattern
...
def get_unique_brands(db: Session) -> List[str]:
    ...
    """
    brands = (
        db.query(models.Phone.brand)
        .distinct()
        .filter(models.Phone.brand.isnot(None))
        .all()
    )
...
def update_device(
    db: Session, device_id: int, device: schemas.PhoneCreate

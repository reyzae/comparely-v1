...
from app.models import AppSettings, Category, Phone
...
async def update_api_settings(
    request: Request, ai_api_key: str = Form(...), db: Session = Depends(get_db)
):
    """Update API settings"""
    try:
        ...
        return RedirectResponse(
            url="/admin/settings?error=Failed to update API settings", status_code=303
        )
...
async def backup_database(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        set_setting(
            db, "last_backup_date", datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        )
        ...
        return RedirectResponse(
            url="/admin/settings?error=Failed to backup database", status_code=303
        )
...
async def optimize_database(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        db.commit()
        logger.info(
            f"Database optimization completed: {optimized_count} tables optimized"
        )
        ...
        return RedirectResponse(
            url="/admin/settings?error=Failed to optimize database", status_code=303
        )
...
async def reset_database(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        return RedirectResponse(
            url="/admin/settings?error=Failed to reset database", status_code=303
        )
...
async def clear_cache(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        return RedirectResponse(
            url=f"/admin/settings?message={message}", status_code=303
        )
    except Exception as e:
        logger.exception(f"Error clearing cache: {e}")
        return RedirectResponse(
            url="/admin/settings?error=Failed to clear cache", status_code=303
        )
...
async def update_ui_preferences(
    ...
    try:
        ...
        set_setting(
            db, "items_per_page", str(items_per_page), "Number of items per page"
        )
        set_setting(db, "date_format", date_format, "Date format for display")

        logger.info(
            f"UI preferences updated: {items_per_page} items per page, {date_format} date format"
        )
        ...
        return RedirectResponse(
            url="/admin/settings?error=Failed to update UI preferences", status_code=303
        )

...
async def admin_login(
    ...
    if not user:
        return RedirectResponse(
            url="/admin/login?error=Invalid username or password", status_code=303
        )

    # Check if user is active
    if not user.is_active:
        return RedirectResponse(
            url="/admin/login?error=Account is disabled", status_code=303
        )
...
async def admin_profile(request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse(
        "admin/profile.html", {"request": request, "current_user": current_user}
    )
...
async def admin_change_password(
    ...
    if new_password != confirm_password:
        return RedirectResponse(
            url="/admin/profile?error=New passwords do not match", status_code=303
        )
    ...
    if not user or not verify_password(current_password, user.password_hash):
        return RedirectResponse(
            url="/admin/profile?error=Current password is incorrect", status_code=303
        )

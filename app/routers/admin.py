...
from app.models import Category, Phone, Role, User
...
async def admin_login(request: Request, db: Session = Depends(get_db)):
    ...
    if username and password:
        ...
        return RedirectResponse(
            url="/admin/login?error=Please enter username and password", status_code=303
        )
...
async def admin_dashboard(request: Request, db: Session = Depends(get_db)):
    ...
    # Get latest year
    latest_year = (
        db.query(Phone.release_year).order_by(Phone.release_year.desc()).first()
    )
...
async def admin_categories(
    ...
    for cat in categories:
        ...
        category_stats.append({"category": cat, "device_count": device_count})
...
async def admin_devices(
    ...
    if search:
        query = query.filter(
            (Phone.name.ilike(f"%{search}%")) | (Phone.brand.ilike(f"%{search}%"))
        )
    ...
    years = (
        ...
        .distinct()
        .filter(Phone.release_year.isnot(None))
...
async def admin_category_new(request: Request):
    ...
    return templates.TemplateResponse(
        "admin/category_form.html", {"request": request, "category": None}
    )
...
async def admin_user_new(request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse(
        "admin/user_form.html", {"request": request, "user": None, "roles": roles}
    )
...
async def admin_user_edit(
    ...
    roles = db.query(Role).all()
    return templates.TemplateResponse(
        "admin/user_form.html", {"request": request, "user": user, "roles": roles}
    )
...
async def admin_role_new(request: Request):
    ...
    return templates.TemplateResponse(
        "admin/role_form.html", {"request": request, "role": None}
    )
...
async def admin_role_edit(
    ...
    return templates.TemplateResponse(
        "admin/role_form.html", {"request": request, "role": role}
    )
...
async def admin_change_password(
    ...
    return RedirectResponse(
        url="/admin/profile?message=Password changed successfully", status_code=303
    )
...
async def admin_category_create(
    ...
    try:
        ...
        return RedirectResponse(
            url=f"/admin/categories?error={str(e)}", status_code=303
        )
...
async def admin_category_update(
    ...
    try:
        ...
        if not category:
            return RedirectResponse(
                url="/admin/categories?error=Category not found", status_code=303
            )
...
async def admin_categories_bulk_delete(
    request: Request, category_ids: str = Form(...), db: Session = Depends(get_db)
):
    ...
    try:
        ...
        return RedirectResponse(
            url="/admin/categories?error=Failed to delete categories", status_code=303
        )
...
async def admin_user_create(
    ...
    try:
        ...
        if existing_user:
            return RedirectResponse(
                url="/admin/users?error=Username already exists", status_code=303
            )
        ...
        return RedirectResponse(
            url=f"/admin/users?error=An error occurred. Please try again.",
            status_code=303,
        )
...
async def admin_role_create(
    ...
    try:
        ...
        if (
            ...
            and permissions.strip().lower() != "none"
        ):
...
async def admin_stats(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        brand_stats = dict(
            sorted(brand_stats.items(), key=lambda x: x[1], reverse=True)
        )
...
async def admin_analytics(request: Request, db: Session = Depends(get_db)):
    ...
    price_ranges = {"< 2 Juta": 0, "2-5 Juta": 0, "5-10 Juta": 0, "> 10 Juta": 0}
...
async def admin_activity_logs(
    request: Request, page: int = 1, action_type: str = None, entity_type: str = None
):
...
async def bulk_import_devices(request: Request, db: Session = Depends(get_db)):
    ...
    if not csv_file:
        return RedirectResponse(
            url="/admin/bulk-operations?error=No file uploaded", status_code=303
        )
    ...
    try:
        ...
        for row in csv_reader:
            ...
            device = Phone(
                ...
                release_year=(
                    int(row.get("release_year")) if row.get("release_year") else None
                ),
        ...
        return RedirectResponse(
            url=f"/admin/bulk-operations?error=Import failed: {str(e)}", status_code=303
        )
...
async def bulk_assign_category(
    ...
    try:
        ...
        return RedirectResponse(
            url=f"/admin/bulk-operations?error={str(e)}", status_code=303
        )
...
async def update_api_settings(request: Request, ai_api_key: str = Form(...)):
    """Update API settings"""
    import os
...
async def backup_database(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        return RedirectResponse(
            url=f"/admin/settings?error=Backup failed: {str(e)}", status_code=303
        )
...
async def update_ui_preferences(request: Request):
    ...
    return RedirectResponse(
        url="/admin/settings?message=UI preferences updated", status_code=303
    )
...
async def clear_cache(request: Request):
    ...
    return RedirectResponse(
        url="/admin/settings?message=Cache cleared successfully", status_code=303
    )

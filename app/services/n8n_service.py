...
def send_comparison_to_n8n(
    device1: models.Phone, 
    device2: models.Phone,
    rule_based_highlights: List[str]
...
def process_n8n_response(n8n_response: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        for highlight in ai_highlights:
            if isinstance(highlight, dict) and all(k in highlight for k in ["category", "winner", "reason"]):
...
def merge_highlights(
    rule_based_highlights: List[str],
    ai_highlights: List[Dict[str, str]]
) -> List[Dict[str, Any]]:
    """
